...
CODA="cmd=/home/adam/chess/engines/Stockfish/src/stockfish proto=uci option.Threads=1 option.Hash=128"
OPP=(
...
run_match() {
  ...
  $CC -recover -concurrency $CONC -rounds $ROUNDS -games 2 \
    ...
    $tcargs \
    -engine name=SF dir=. $CODA \
    -engine name=$name $oeng \
    -pgnout $OUT/${cond}_${name}.pgn \
    2>&1 | tail -40 | grep -iE "Score of SF|Elo difference" | tail -2 | sed "s/^/  [$cond vs $name] /"
}
