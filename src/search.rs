...
pub(crate) fn search_helper(board: &mut Board, info: &mut SearchInfo, _limits: &SearchLimits, thread_id: usize) -> Move {
    ...
    for depth in 1..=effective_max {
        ...
        if depth >= 4 && prev_score > -MATE_IN_MAX_PLY && prev_score < MATE_IN_MAX_PLY {
            ...
            // Keep the width, but move half a score-trend step toward the
            // likelier failure direction.
            let center = prev_score + ((prev_score - prev_prev_score) / 2).clamp(-24, 24);
            let mut alpha = (center - delta).max(-INFINITY);
            let mut beta = (center + delta).min(INFINITY);
