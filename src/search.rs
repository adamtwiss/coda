...
tunables!(
    ...
    // E3 retry (half scale): any_threat_count × SE margin widener.
    // Original #590 at scale=2 landed -1.6; mechanism likely right,
    // over-aggressive. Halving to 1 aligns with king-pressure scale (3).
    (SE_THREATS_MARGIN, 1, 0, 10, 1.0),
    ...
    loop {
            ...
            && FEAT_SINGULAR.load(Ordering::Relaxed)
        {
            ...
            if tt_score_local > -(MATE_SCORE - 100) && tt_score_local < MATE_SCORE - 100 {
                ...
                // E3-half: further widen per any_threat_count (#590 retry).
