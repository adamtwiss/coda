...
fn negamax(
    ...
    let mut quiet_reduced_only = [false; 64];
    ...
    loop {
        ...
        if let Some(slot) = quiet_slot {
            quiet_reduced_only[slot] = reduced_fail_low;
        }
                ...
                if alpha >= beta {
                    ...
                    if !is_cap && !is_promo {
                        ...
                        for i in 0..quiets_count.saturating_sub(1) {
                            // Opposite-direction experiment: strengthen the final
                            // penalty only for reduced-only quiet failures.
                            // Re-searched and extended moves retain baseline malus.
                            let malus = if quiet_reduced_only[i] { malus * 5 / 4 } else { malus };
