...
tunables!(
    // v9 #796 tune applied (2500 iters, on C8fix-2 factor SB200 net CC483681).
    // Full 77-param sweep starting from #784 values, retuning for the
    // clean-threat eval distribution after the C8 x-ray fix completed.
    //
    // BRANCH ONLY — main remains tuned for 1EF1C3E5 (PROD). Use this branch
    // for SPRTs against CC483681 to isolate C8fix-2's contribution from the
    // tunable-mismatch confound that #794 hit.
    //
    // Big movers vs #784 (1EF1C3E5-tuned trunk):
    //   NMP_EVAL_MAX 3→2, LMR_THREAT_DIV 3→2 — clean threat signals trusted more
    //   HIST_PRUNE_DEPTH 5→4, HINDSIGHT_MIN_DEPTH 4→3 — shallower aggressive pruning
    //   CORR_W_MINOR 65→51, CORR_W_NP 71→62 — clean eval more accurate, less correction
    //   CORR_W_CONT 38→45 — lean MORE on continuation-history correction
    //   CORR_BONUS_CAP_DIV 2→3, CORR_HIST_ERR_MAX 2→3 — corrhist learning retuned
    //   NMP_BASE_R 6→7, NMP_MIN_DEPTH 6→7 — NMP later, more reduction
    //   PROBCUT_KING_ZONE_MAX 6→8, PROBCUT_MIN_DEPTH 6→5 — probcut earlier/wider
    //   SEE_QUIET_MULT 24→28, FUT_THREATS_MARGIN 38→45 — pruning shifts
    //   CAP_HIST_BASE 18→14, HIST_BONUS_OFFSET 80→66 — history shape retuned
    //
    // Overrides applied to SPSA output:
    //   LMR_ENDGAME_PIECES kept at 5 (play-quality load-bearing).
    (NMP_BASE_R, 7, 2, 8, 1.5),
    (NMP_DEPTH_DIV, 3, 1, 6, 1.5),
    (NMP_EVAL_DIV, 99, 50, 400, 17.5),
    (NMP_EVAL_MAX, 2, 1, 6, 1.5),
    (NMP_VERIFY_DEPTH, 9, 4, 20, 2.0),
    (RFP_DEPTH, 19, 2, 20, 2.0),
    (RFP_MARGIN_IMP, 34, 30, 150, 6.0),
    ...
    (FUT_BASE, 38, 20, 200, 9.0),
    (FUT_PER_DEPTH, 68, 40, 250, 10.5),
    (HIST_PRUNE_DEPTH, 4, 1, 8, 1.5),
    (HIST_PRUNE_MULT, 13361, 500, 50000, 2475.0),
    (SEE_QUIET_MULT, 28, 5, 80, 3.75),
    (LMR_HIST_DIV, 7103, 2000, 100000, 4900.0),
    (LMR_C_QUIET, 134, 40, 300, 13.0),
    (LMR_C_CAP, 109, 80, 350, 12.5),
    (SE_DEPTH, 5, 4, 20, 2.0),
    (ASP_DELTA, 13, 5, 30, 1.5),
    (ASP_SCORE_DIV, 34480, 8000, 50000, 2100.0),
    (LMP_BASE, 9, 1, 15, 2.0),
    (LMP_DEPTH, 12, 4, 20, 2.0),
    (BAD_NOISY_MARGIN, 77, 30, 150, 6.0),
    (PROBCUT_MARGIN, 201, 80, 300, 11.0),
    (HINDSIGHT_THRESH, 162, 50, 400, 17.5),
    (UNSTABLE_THRESH, 168, 50, 500, 22.5),
    (SEE_MATERIAL_SCALE, 216, 30, 300, 13.5),
    (QS_DELTA_MARGIN, 374, 100, 500, 20.0),
    (QS_SEE_THRESHOLD, -39, -200, 0, 10.0),
    (QS_MAX_CAPTURES, 26, 2, 32, 2.0),
    (CORR_W_PAWN, 272, 100, 600, 25.0),
    (CORR_W_NP, 62, 50, 400, 17.5),
    (CORR_W_MINOR, 51, 30, 300, 13.5),
    (CORR_W_MAJOR, 105, 30, 300, 13.5),
    (CORR_W_CONT, 45, 30, 400, 18.5),
    (FH_BLEND_DEPTH, 1, 0, 8, 1.5),
    (HIST_BONUS_MULT, 328, 50, 400, 17.5),
    ...
    (HIST_BONUS_OFFSET, 66, 0, 400, 25.0),
    (CAP_HIST_MULT, 286, 50, 400, 17.5),
    (CAP_HIST_BASE, 14, 0, 200, 10.0),
    (CAP_HIST_MAX, 1744, 500, 3000, 125.0),
    (DEXT_MARGIN, 13, 2, 50, 2.4),
    (DEXT_CAP, 14, 4, 32, 2.0),
    (QUIET_CHECK_BONUS, 12788, 2000, 30000, 1400.0),
    (LMR_COMPLEXITY_DIV, 200, 30, 500, 23.5),
    (CORR_HIST_DIV, 952, 256, 4096, 192.0),
    (CORR_UPDATE_WEIGHT_MAX, 15, 4, 48, 2.2),
    (CORR_BONUS_CAP_DIV, 3, 1, 16, 1.5),
    (CORR_HIST_GRAIN_T, 11, 1, 32, 1.55),
    (CORR_HIST_ERR_MAX, 3, 1, 64, 3.15),
    (ESCAPE_BONUS_Q, 14597, 5000, 40000, 1750.0),
    (ESCAPE_BONUS_R, 12437, 3000, 30000, 1350.0),
    (ESCAPE_BONUS_MINOR, 4877, 2000, 20000, 900.0),
    ...
    (MOBILITY_DELTA_WEIGHT, 31, 0, 256, 8.0),
    (PROBCUT_KING_ZONE_MAX, 8, 2, 9, 1.5),
    (LMR_THREAT_DIV, 2, 1, 5, 1.5),
    (LMR_KING_PRESSURE_DIV, 5, 2, 9, 1.5),
    (FUT_THREATS_MARGIN, 45, 0, 200, 10.0),
    (DISCOVERED_ATTACK_BONUS, 6593, 0, 30000, 1500.0),
    ...
    (BATTERY_BONUS, 7736, 0, 20000, 1000.0),
    ...
    (QSEE_BONUS, 4735, 0, 20000, 1000.0),
    ...
    (KNIGHT_FORK_BONUS, 9764, 0, 20000, 1000.0),
