...
tunables!(
    ...
    (NMP_BASE_R, 6, 2, 8),
    (NMP_DEPTH_DIV, 3, 1, 6),
    (NMP_EVAL_DIV, 100, 100, 400),
    (NMP_EVAL_MAX, 5, 1, 6),
    (NMP_VERIFY_DEPTH, 15, 8, 20),
    // RFP
    (RFP_DEPTH, 10, 2, 12),
    (RFP_MARGIN_IMP, 60, 30, 150),
    (RFP_MARGIN_NOIMP, 106, 50, 200),
    // Futility
    (FUT_BASE, 59, 20, 200),
    (FUT_PER_DEPTH, 119, 40, 250),
    // History pruning
    (HIST_PRUNE_DEPTH, 5, 1, 8),
    (HIST_PRUNE_MULT, 4842, 500, 50000),
    // SEE pruning
    (SEE_QUIET_MULT, 30, 5, 80),
    (SEE_CAP_MULT, 153, 30, 200),
    // LMR
    (LMR_HIST_DIV, 8386, 2000, 100000),
    (LMR_C_QUIET, 128, 40, 300),
    (LMR_C_CAP, 100, 100, 350),
    // Singular extensions
    (SE_DEPTH, 4, 4, 20),
    // Aspiration windows
    (ASP_DELTA, 13, 5, 30),
    (ASP_SCORE_DIV, 33950, 8000, 50000),
    // LMP
    (LMP_BASE, 11, 1, 15),
    (LMP_DEPTH, 9, 4, 20),
    // Bad noisy
    (BAD_NOISY_MARGIN, 124, 30, 150),
    // ProbCut
    (PROBCUT_MARGIN, 195, 80, 300),
    // Hindsight
    (HINDSIGHT_THRESH, 182, 50, 400),
    // Unstable position detection
    (UNSTABLE_THRESH, 173, 50, 500),
    // SEE piece value scaling
    (SEE_MATERIAL_SCALE, 194, 30, 300),
    // QS
    (QS_DELTA_MARGIN, 388, 100, 500),
    (QS_SEE_THRESHOLD, -38, -200, 0),
    (QS_MAX_CAPTURES, 26, 2, 32),
    // Correction history weights
    (CORR_W_PAWN, 302, 100, 600),
    (CORR_W_NP, 119, 50, 400),
    (CORR_W_MINOR, 59, 30, 300),
    (CORR_W_MAJOR, 105, 30, 300),
    (CORR_W_CONT, 32, 30, 400),
    // Fail-high blend
    (FH_BLEND_DEPTH, 0, 0, 8),
    // History bonus
    (HIST_BONUS_MULT, 292, 50, 400),
    (HIST_BONUS_MAX, 1588, 500, 3000),
    // Capture history bonus
    (CAP_HIST_MULT, 252, 50, 400),
    (CAP_HIST_BASE, 7, 0, 200),
    (CAP_HIST_MAX, 1664, 500, 3000),
    // Double extensions
    (DEXT_MARGIN, 11, 2, 50),
    (DEXT_CAP, 18, 4, 32),
    // Quiet check bonus
    (QUIET_CHECK_BONUS, 9755, 2000, 30000),
    // LMR complexity
    (LMR_COMPLEXITY_DIV, 178, 30, 500),
    // Contempt
    // Correction history divisor
    (CORR_HIST_DIV, 1304, 256, 4096),
    // Correction history update weight cap.
    (CORR_UPDATE_WEIGHT_MAX, 18, 4, 48),
    (CORR_BONUS_CAP_DIV, 3, 1, 16),
    (CORR_HIST_GRAIN_T, 11, 1, 32),
    ...
    (ESCAPE_BONUS_Q, 16441, 5000, 40000),
    (ESCAPE_BONUS_R, 12901, 3000, 30000),
    (ESCAPE_BONUS_MINOR, 9848, 2000, 20000),
    // v9 threat-family gates/modifiers (v9-specific — require threat-aware net).
    (NMP_KING_ZONE_MAX, 6, 2, 9),
    (PROBCUT_KING_ZONE_MAX, 6, 2, 9),
    (LMR_THREAT_DIV, 3, 1, 5),
    ...
    (DISCOVERED_ATTACK_BONUS, 6703, 0, 30000),
