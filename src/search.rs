...
fn negamax(
    ...
    loop {
        ...
        // A reduced-only fail-low provides weaker negative evidence.
        // Preserve this move's verdict before later moves are searched.
        if let Some(slot) = quiet_slot {
            quiet_malus_relief[slot] = reduced_fail_low;
        }
