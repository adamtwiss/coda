    ...
    loop {
        ...
        if !in_check && is_cap && !is_promo && move_count > 1 && mv != tt_move && !is_endgame_skip && FEAT_LMR.load(Ordering::Relaxed) {
            ...
            if beta - alpha == 1 {
                ...
                // Capture history is already a continuous confidence signal.
                // Let a sufficiently negative history lift a sub-ply table
                // value across the one-ply LMR boundary. Existing reductions
                // retain the exact old modifier order.
                let base_reduces = reduction >= LMR_SCALE;
                if !base_reduces && moved_piece != NO_PIECE && captured_pt != NO_PIECE_TYPE {
                    ...
                    reduction -= capt_hist_val * LMR_SCALE / tp(&LMR_HIST_DIV_CAP);
                }
                ...
                if reduction >= LMR_SCALE {
                    ...
                    // capt_hist signal.
                    //
                    // Capture history has already been applied when it was
                    // needed to cross the gate; avoid charging it twice.
                    if base_reduces && moved_piece != NO_PIECE && captured_pt != NO_PIECE_TYPE {
                        let ct = if flags == FLAG_EN_PASSANT { captured_type(PAWN) } else { captured_type(captured_pt) };
                        let capt_hist_val = info.history.capture[go_piece(moved_piece)][to as usize][ct] as i32;
