...
tunables!(
    // v9 post-#554 r1 retune (tune #562, 1000 iters, post-merge of
    // SE-king-pressure, ProbCut-skip, NMP-skip, offense-bonus, SMP fix).
    // Biggest movers: LMR_HIST_DIV +41.7%, CORR_HIST_ERR_MAX +20.1%,
    // CORR_W_MINOR +16.8%, SE_DEPTH -15.8%, CORR_W_CONT -14.5%,
    // HIST_PRUNE_MULT +14.4%, FUT_THREATS_MARGIN +11.7%, LMR_COMPLEXITY_DIV -10.8%.
    (NMP_BASE_R, 5, 2, 8),
    (NMP_DEPTH_DIV, 3, 1, 6),
    (NMP_EVAL_DIV, 117, 100, 400),
    (NMP_EVAL_MAX, 5, 1, 6),
    ...
    (HIST_PRUNE_MULT, 4776, 500, 50000),
    // SEE pruning
    (SEE_QUIET_MULT, 43, 5, 80),
    (SEE_CAP_MULT, 146, 30, 200),
    // LMR
    (LMR_HIST_DIV, 10742, 2000, 100000),
    (LMR_C_QUIET, 127, 40, 300),
    (LMR_C_CAP, 101, 100, 350),
    // Singular extensions
    (SE_DEPTH, 4, 4, 20),
    // Aspiration windows
    (ASP_DELTA, 12, 5, 30),
    (ASP_SCORE_DIV, 33541, 8000, 50000),
    // LMP
    (LMP_BASE, 13, 1, 15),
    (LMP_DEPTH, 9, 4, 20),
    // Bad noisy
    (BAD_NOISY_MARGIN, 128, 30, 150),
    // ProbCut
    (PROBCUT_MARGIN, 190, 80, 300),
    // Hindsight
    ...
    // QS
    (QS_DELTA_MARGIN, 366, 100, 500),
    (QS_SEE_THRESHOLD, -30, -200, 0),
    (QS_MAX_CAPTURES, 26, 2, 32),
    // Correction history weights
    (CORR_W_PAWN, 297, 100, 600),
    (CORR_W_NP, 105, 50, 400),
    (CORR_W_MINOR, 72, 30, 300),
    (CORR_W_MAJOR, 97, 30, 300),
    (CORR_W_CONT, 35, 30, 400),
    // Fail-high blend
    (FH_BLEND_DEPTH, 1, 0, 8),
    // History bonus
    (HIST_BONUS_MULT, 303, 50, 400),
    (HIST_BONUS_MAX, 1565, 500, 3000),
    // Capture history bonus
    (CAP_HIST_MULT, 254, 50, 400),
    (CAP_HIST_BASE, 15, 0, 200),
    (CAP_HIST_MAX, 1663, 500, 3000),
    // Double extensions
    (DEXT_MARGIN, 10, 2, 50),
    (DEXT_CAP, 18, 4, 32),
    // Quiet check bonus
    (QUIET_CHECK_BONUS, 9405, 2000, 30000),
    // LMR complexity
    (LMR_COMPLEXITY_DIV, 164, 30, 500),
    // Contempt
    ...
    (ESCAPE_BONUS_Q, 15125, 5000, 40000),
    (ESCAPE_BONUS_R, 13432, 3000, 30000),
    (ESCAPE_BONUS_MINOR, 10049, 2000, 20000),
    ...
    (DISCOVERED_ATTACK_BONUS, 8351, 0, 30000),
