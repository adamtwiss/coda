...
tunables!(
    // v9 post-#555 retune (tune #564, 1000 iters, on experiment/rook-kingring-bonus).
    // Retune-on-branch rescue attempt. Biggest movers vs tune-spec starts:
    // FUT_THREATS_MARGIN -27.4% (OPPOSITE to #562's +11.7%), CORR_HIST_ERR_MAX
    // -24.2% (OPPOSITE to #562's +20.1%), CAP_HIST_BASE -21.9%, MVV_CAP_MULT
    // +14.5%, DISCOVERED_ATTACK_BONUS -14.0%, HIST_PRUNE_MULT +13.3%.
    // The opposite-direction moves vs #562 indicate the rook-kingring feature
    // genuinely pulls the pruning landscape differently — validates the
    // retune-on-branch hypothesis for this feature.
    (NMP_BASE_R, 5, 2, 8),
    (NMP_DEPTH_DIV, 3, 1, 6),
    (NMP_EVAL_DIV, 119, 100, 400),
    (NMP_EVAL_MAX, 5, 1, 6),
    (NMP_VERIFY_DEPTH, 12, 8, 20),
    // RFP
    (RFP_DEPTH, 7, 2, 12),
    (RFP_MARGIN_IMP, 81, 30, 150),
    (RFP_MARGIN_NOIMP, 132, 50, 200),
    // Futility
    (FUT_BASE, 68, 20, 200),
    (FUT_PER_DEPTH, 160, 40, 250),
    // History pruning
    (HIST_PRUNE_DEPTH, 4, 1, 8),
    (HIST_PRUNE_MULT, 4730, 500, 50000),
    // SEE pruning
    (SEE_QUIET_MULT, 44, 5, 80),
    (SEE_CAP_MULT, 147, 30, 200),
    // LMR
    (LMR_HIST_DIV, 7215, 2000, 100000),
    (LMR_C_QUIET, 125, 40, 300),
    (LMR_C_CAP, 101, 100, 350),
    // Singular extensions
    (SE_DEPTH, 5, 4, 20),
    // Aspiration windows
    (ASP_DELTA, 12, 5, 30),
    (ASP_SCORE_DIV, 32386, 8000, 50000),
    // LMP
    (LMP_BASE, 13, 1, 15),
    (LMP_DEPTH, 9, 4, 20),
    // Bad noisy
    ...
    // Hindsight
    (HINDSIGHT_THRESH, 177, 50, 400),
    // Unstable position detection
    (UNSTABLE_THRESH, 159, 50, 500),
    // SEE piece value scaling
    (SEE_MATERIAL_SCALE, 197, 30, 300),
    // QS
    (QS_DELTA_MARGIN, 373, 100, 500),
    (QS_SEE_THRESHOLD, -34, -200, 0),
    (QS_MAX_CAPTURES, 27, 2, 32),
    // Correction history weights
    (CORR_W_PAWN, 296, 100, 600),
    (CORR_W_NP, 115, 50, 400),
    (CORR_W_MINOR, 58, 30, 300),
    (CORR_W_MAJOR, 94, 30, 300),
    (CORR_W_CONT, 39, 30, 400),
    // Fail-high blend
    (FH_BLEND_DEPTH, 1, 0, 8),
    // History bonus
    (HIST_BONUS_MULT, 305, 50, 400),
    (HIST_BONUS_MAX, 1589, 500, 3000),
    // Capture history bonus
    (CAP_HIST_MULT, 253, 50, 400),
    (CAP_HIST_BASE, 12, 0, 200),
    (CAP_HIST_MAX, 1617, 500, 3000),
    // Double extensions
    (DEXT_MARGIN, 10, 2, 50),
    (DEXT_CAP, 19, 4, 32),
    // Quiet check bonus
    (QUIET_CHECK_BONUS, 8745, 2000, 30000),
    // LMR complexity
    (LMR_COMPLEXITY_DIV, 186, 30, 500),
    // Contempt
    // Correction history divisor
    (CORR_HIST_DIV, 1345, 256, 4096),
    // Correction history update weight cap.
    (CORR_UPDATE_WEIGHT_MAX, 18, 4, 48),
    ...
    (ESCAPE_BONUS_Q, 15142, 5000, 40000),
    (ESCAPE_BONUS_R, 13174, 3000, 30000),
    (ESCAPE_BONUS_MINOR, 9864, 2000, 20000),
    ...
    (DISCOVERED_ATTACK_BONUS, 6877, 0, 30000),
