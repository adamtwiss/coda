...
fn negamax(
    ...
    loop {
        ...
        // With fixed history (27× larger), values now reach meaningful thresholds.
        let mut extension = 0;
        if !is_cap && moved_piece != NO_PIECE && ply > 0 {
            let gp = go_piece(moved_piece);
            let stack_len = board.undo_stack.len();
            let mut strong_hist = true;
            for &off in &[1usize, 2] {
                if stack_len >= off {
                    ...
                    if undo.mv != NO_MOVE {
                        ...
                        } else { strong_hist = false; break; }
                    } else { strong_hist = false; break; }
                } else { strong_hist = false; break; }
            }
            if strong_hist { extension = 1; }
        }
