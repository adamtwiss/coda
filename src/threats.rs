...
pub fn threat_index(
    ...
    unsafe {
        ...
        // Semi-exclusion uses PHYSICAL squares (matching Bullet training).
        // This ensures training and inference agree on which semi-excluded
        // pairs to keep, regardless of perspective flip direction.
        let base = pair.base(from, to);
        if base < 0 {
            return base; // excluded or semi-excluded pair
        }

        // Perspective flip for index computation (not semi-exclusion)
