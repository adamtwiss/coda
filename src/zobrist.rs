...
/// Go's math/rand LCG PRNG (matches the default source).
/// This produces the exact same sequence as Go's rand.New(rand.NewSource(seed)).
struct GoRng {
    state: i64,
}
...
impl GoRng {
    fn new(seed: i64) -> Self {
        // Go's rand.NewSource(seed) uses seed directly as initial state
        // but the actual implementation uses a more complex scheme.
        // For exact match, we use pre-computed keys instead.
        GoRng { state: seed }
    }
}
