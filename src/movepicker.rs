...
impl MovePicker {
    ...
    fn generate_and_score_captures(&mut self, board: &Board) {
        ...
        for i in 0..caps.len {
            ...
            let cap_score = mvv_lva(board, m) + capt_hist_score_static(board, history, m);
            // Dynamic SEE threshold: well-historied captures tolerate slight material loss
            // Consensus: -score/32 (Obsidian/Alexandria pattern)
            let see_threshold = (-cap_score / 32).min(0);
