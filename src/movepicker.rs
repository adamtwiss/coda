...
impl MovePicker {
    ...
    fn generate_and_score_quiets(&mut self, board: &Board) {
        ...
        for i in 0..quiets.len {
            ...
            // square. Tunable (QUEEN_7TH_BONUS), gated on safety.
            let q7_bonus = crate::search::QUEEN_7TH_BONUS.load(std::sync::atomic::Ordering::Relaxed);
            if q7_bonus > 0 && piece != NO_PIECE {
                let pt = board.piece_type_at(from);
                if pt == 4 {  // queen
