...
/// Material values (midgame, endgame) in centipawns.
...
/// PeSTO piece-square tables (from side-to-move perspective, a1=0).
/// Midgame tables.
#[rustfmt::skip]
    ...
    for color in 0..2u8 {
        for pt in 0..6u8 {
            ...
            while bb != 0 {
                ...
                let table_sq = if color == WHITE { sq } else { flip_sq(sq) };

                mg[color as usize] += MG_VALUE[pt as usize] + MG_TABLES[pt as usize][table_sq as usize];
                eg[color as usize] += EG_VALUE[pt as usize] + EG_TABLES[pt as usize][table_sq as usize];
