...
/// Derive a cache key that mixes halfmove into the Zobrist hash. Uses a
/// SplitMix-style scramble of halfmove so adjacent halfmove values hash to
/// well-separated slots (avoids clustered collisions on near-50mr positions).
#[inline(always)]
fn effective_key(key: u64, halfmove: u16) -> u64 {
    // SplitMix64 step on halfmove, then XOR into the Zobrist key.
    let mut h = halfmove as u64;
...
impl TbCache {
    ...
    /// Store a probe result keyed by Zobrist + halfmove. No-op when disabled.
    #[inline]
    pub fn store(&self, key: u64, halfmove: u16, wdl: i32) {
...
mod tests {
    ...
    fn disabled_cache_never_hits() {
        let c = TbCache::new(0);
        assert!(!c.enabled());
        assert_eq!(c.probe(0xDEADBEEF, 0), None);
        c.store(0xDEADBEEF, 0, 2); // no-op
        assert_eq!(c.probe(0xDEADBEEF, 0), None);
    }
    ...
    fn store_probe_roundtrip() {
        ...
        for wdl in [-20000, -1, 0, 1, 20000] {
            ...
            c.store(key, 0, wdl);
            assert_eq!(c.probe(key, 0), Some(wdl), "roundtrip failed for wdl={}", wdl);
        }
    }
    ...
    fn collision_different_key_misses() {
        ...
        assert_eq!(effective_key(colliding, 0) & mask, effective_key(0xAAAA, 0) & mask);
        assert_eq!(c.probe(colliding, 0), None, "collision must not hit");
    }
    ...
    fn clear_removes_entries() {
        let c = TbCache::new(1);
        c.store(0x1234, 0, -1);
        assert_eq!(c.probe(0x1234, 0), Some(-1));
        c.clear();
        assert_eq!(c.probe(0x1234, 0), None);
    }
    ...
    fn different_halfmove_misses() {
        ...
        assert_eq!(c.probe(0x1234_5678, 0), Some(20000));
