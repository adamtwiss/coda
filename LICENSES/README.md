# Referenced engines and their licenses

This directory records the open-source chess engines Coda studied and credits as
sources of code or ideas, grouped by license.

A copy of the Open Database License v1 is attached, for the use of data provided by the Leela Chess Zero team, located here: https://storage.lczero.org/files/training_data

## AGPL-3.0

Same copyleft as Coda's AGPL-3.0 license.

| Engine | Source |
|--------|--------|
| Icarus | https://github.com/Sp00ph/icarus |
| Raphael | https://github.com/Orbital-Web/Raphael |
| Reckless | https://github.com/codedeliveryservice/Reckless |
| Tcheran | https://github.com/jgilchrist/tcheran |
| Viridithas | https://github.com/cosmobobak/viridithas |

## GPL-3.0

Compatible with Coda's AGPL-3.0 distribution (via §13 of both licenses).

| Engine | Source |
|--------|--------|
| Alexandria | https://github.com/PGG106/Alexandria |
| Astra | https://github.com/h1me01/AstraC |
| Berserk | https://github.com/jhonnold/berserk |
| BlackMarlin | https://github.com/dsekercioglu/blackmarlin |
| Cinder | https://github.com/brunocodutra/cinder |
| Clarity | https://github.com/Vast342/Clarity |
| Clover | https://github.com/lucametehau/CloverEngine |
| Ethereal | https://github.com/AndyGrant/Ethereal |
| Halogen | https://github.com/KierenP/Halogen |
| Horsie | https://github.com/liamt19/Horsie |
| Igel | https://github.com/vshcherbyna/igel |
| Koivisto | https://github.com/Luecx/Koivisto |
| Laser | https://github.com/jeffreyan11/uci-chess-engine |
| Minic | https://github.com/tryingsomestuff/Minic |
| Obsidian | https://github.com/gab8192/Obsidian |
| PlentyChess | https://github.com/Yoshie2000/PlentyChess |
| Quanticade | https://github.com/DarkNeutrino/Quanticade |
| Rodent | https://github.com/nescitus/Rodent_III |
| RubiChess | https://github.com/Matthies/RubiChess |
| Seer | https://github.com/connormcmonigle/seer-nnue |
| Stockfish | https://github.com/official-stockfish/Stockfish |
| Stormphrax | https://github.com/Ciekce/Stormphrax |
| Tarnished | https://github.com/Bobingstern/Tarnished |
| Texel | https://github.com/peterosterlund2/texel |
| Tucano | https://github.com/alcides-schulz/Tucano |
| Uralochka | https://gitlab.com/freemanzlat/uralochka3 |
| Velvet | https://github.com/mhonert/velvet-chess |
| Weiss | https://github.com/TerjeKir/weiss |
| Winter | https://github.com/rosenthj/Winter |

## MIT

Permissive, compatible with Coda's AGPL-3.0 distribution.

| Engine | Source |
|--------|--------|
| Altair | https://github.com/Alex2262/AltairChessEngine |
| Arasan | https://github.com/jdart1/arasan-chess |
| Caissa | https://github.com/Witek902/Caissa |
| Hobbes | https://github.com/kelseyde/hobbes-chess-engine |
| Midnight | https://github.com/archishou/MidnightChessEngine |
| Wasp | https://waspchess.com/ |

## WTFPL

Public-domain-equivalent, compatible with Coda's AGPL-3.0 distribution.

| Engine | Source |
|--------|--------|
| Starzix | https://github.com/zzzzz151/Starzix |

## All rights reserved (studied only)

The engines below carry no license granting reuse and are treated as **all
rights reserved**. They were studied as part of Coda's cross-engine research
only: **no code was taken from them, and to our knowledge that study did not
result in any derivative output in Coda.**

| Engine | Source |
|--------|--------|
| Crafty | https://craftychess.com/ |
| ExChess | https://github.com/einnuj/ExChess |
| GreKo | https://www.greko.su/index_en.html |
| Integral | https://github.com/aronpetko/integral |
| Motor | https://github.com/martinnovaak/motor |
