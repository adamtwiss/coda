...
~2.9 GB freed. Historical test results preserved (Test.dev_network/base_network
are CharField, not FK — no cascade).

Local .nnue files under `nets/` are unaffected. If any of the purged nets is
ever needed again, re-upload via `ob_upload_net.py` with the local file.
