...
| item | status |
|---|---|
| **H1** 2-ply/4-ply cont-corr | **Zeus — WIN, merge candidate (awaiting Adam nod).** Flat 1-ply `[piece][to]` → paired 4D `[prev][cur]` read at ply-2+ply-4 (ply threaded incl. qsearch; fortress+unit tests green). **Untuned SPRT #2675 H1 +3.3 ±2.2 (24.8k) → MERGE = commit `66c1699`.** The retune-on-branch archetype **INVERTED**: #2676 retune (`CORR_W_CONT` ~unchanged; `CORR_HIST_DIV` 450→526 etc.) → tuned SPRT #2677 **H0 −0.3 ±1.5 (58.8k) — the retune ERASED the win** (cluster already #2664-calibrated; a 9-param STC branch retune drifted off it). Bank the untuned structural change, drop the retune. Next: **H2(+M2)**. |
| **H2** minor-piece-key corr | **Zeus — H0, DROP (bundles M2).** Implemented SF-shape `board.minor_piece_key` = knights+bishops+**kings** (incremental Zobrist + set_fen recompute + parity fuzzer) and a `minor_corr` source (`CORR_W_MINOR`, untuned default 100), keyed off the residual base. Branch `zeus/minor-piece-corr` (commit 2278870), bench 2106033 vs main 2085296 (+1.0%). **Untuned SPRT #2681 [0,3] H0 −3.1 ±2.8 (16.0k), LLR −2.96 — mildly NEGATIVE.** Read: the king-inclusive key is far sparser (heavier table dilution) and its knight/bishop part overlaps the np signal; the king-position increment doesn't pay for the noise. Not retuned (cluster is #2664-calibrated — H1 lesson). Branch not merged. This ALSO resolves **M2** (king-in-key): king-conditioning a zobrist corr bucket is net-negative here. |
| **H3** trans_corr ablation | **RESOLVED — keep.** Ablation #2670 −3.0 H0 (removing trans costs real Elo). |
| **H4** futility \|corr\| | **DROP — regression on retuned base.** Retested as a HARDCODED 0.33·\|corr\| term (no tunable) on post-#2664 main: `zeus/fut-corr-hardcode` (72bcae9), SPRT #2690 [-1,2] **H0 −3.2 ±2.6 (17.8k)**. Worse than the tunable preliminary (#2671 finished +0.4/180k on the PRE-#2664 base). #2664 already absorbed the futility-\|corr\| benefit into FUT_BASE/PER_DEPTH/LMR_DEPTH, so an explicit term now double-counts. No remaining futility gap. |
...
| **L1** collapse two-stage divide | **DONE — neutral hygiene.** Folded `(sum/DIV)/GRAIN_T` → single `sum/5850`; retired the collinear GRAIN_T. There was NO precision loss to recover (`trunc(trunc(x/a)/b)==trunc(x/(a·b))` in Rust) — bench byte-identical. `zeus/corrhist-divide-collapse` (2c1185e). SPRT #2689 [-2,1] H0 −1.0±1.2/74k = same-binary drift noise (provably no-op). Mergeable as hygiene (removes a redundant SPSA dim); a provably-identical change shouldn't have been SPRT'd at all. |
...
So Zeus's corrhist queue resolved as: **H1** (WIN +3.3, merged 66c1699) → **H2 (+M2): H0 #2681, DROP** → **M4: H0 #2688, DROP** → **L1: neutral hygiene #2689, mergeable (byte-identical)** → **H4: H0 −3.2 #2690, DROP**. H7/M1/M3/M5 dropped. **Campaign net = the single H1 win (+3.3) + the L1 cleanup; everything else is a documented dead end.**
