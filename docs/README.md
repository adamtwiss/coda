...
- [`training_methodology_cross_engine_2026-05-01.md`](training_methodology_cross_engine_2026-05-01.md) — **Broad cross-engine training-methodology survey.** 17 engines (12 open-log + 5 closed/inference). Three headline findings: (1) Viridithas `eleison` resolves Hobbes-vs-Viri WDL contradiction — bare ramp-up fails, ramp-up + eval-rescaling = +9.27 Elo VLTC. (2) Tarnished bigger-arch teacher relabeling — 4096-FT preceptor rescoring 22B corpus. (3) **`arasan-chess/arasanv5.rs` ships a complete Bullet training config** — WDL=0.0, factoriser, FT clip ±0.99, 100× LR decay. Plus QA=181 re-quantisation (+16.76 fixed-nodes), L1-on-activations sparsity (+5.9 LTC), Halogen TD-bootstrap pipeline, Halogen int8 threat weights, SF dual-net architecture, Alexandria's confirmed dual-activation FT. 12 ranked Coda-actionable patches.
- [`training_patterns_2026-04-30.md`](training_patterns_2026-04-30.md) — **Cross-engine NNUE training-time hyperparameter synthesis.** WDL convention (Bullet `wdl = outcome weight`, Lc0 lambda flips), WDL fixed blends (Coda V5=0.07, V9=0.15) vs Hobbes-style ramp schedules (engine-dependent: Hobbes positive, Viridithas negative), LR schedules (Hobbes 8.1e-6 endpoint contradicts Coda's "lower is better"), two-stage training (Hobbes pattern: broad data + cosine + low WDL → recent slice + constant LR + high WDL), fen-skipping, DFRC blend, opening eval-bound filter, activation-chain evolution (Hobbes h-40 dual-activation FT +9.5 Elo). 8 missing/untested patterns in Coda; cumulative envelope ~+10-30 Elo. Test plan parked on factor variance.
