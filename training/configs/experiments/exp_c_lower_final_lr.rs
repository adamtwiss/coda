/// Experiment A: Baseline v7 768pw with SCReLU fix on hidden layers
/// This is the control — same as our old 768pw config but with ReLU→SCReLU fix.
///
/// Run: cargo run --release --example exp_a_baseline_screlu -- \
///   --dataset /training/sf/test80-2024-01-jan-2tb7p.min-v2.v6.binpack \
///   --superbatches 100 --warmup 10
...
fn main() {
    ...
    let final_lr: f32 = 0.001 * 0.3f32.powi(5); // match Bullet example // our standard final LR
    ...
    let mut trainer = ValueTrainerBuilder::default()
        ...
        .build(|builder, stm_inputs, ntm_inputs, output_buckets| {
            ...
            // FT: CReLU → pairwise (standard)
            // Hidden: SCReLU on L1 AND L2 (FIX: was ReLU in old config)
    ...
    let schedule = TrainingSchedule {
        net_id: "exp-c-low-final-lr".to_string(),
        eval_scale: 400.0,
        steps: TrainingSteps {
            batch_size: 16_384,
    ...
    println!("=== Experiment C: Lower final LR ===");
    println!("768pw → CReLU → pairwise → SCReLU L1(16) → SCReLU L2(32) → 1×8");
    println!("WDL: 0.0, LR: {} → {}, Warmup: {} SBs", initial_lr, final_lr, warmup_sbs);
