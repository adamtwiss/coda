/// Experiment F: Dynamic WDL — more WDL for decisive positions
/// Uses wdl_adjust_function to scale WDL proportion with |score|.
/// Quiet positions (|score|<100) get minimal WDL, decisive positions
/// (|score|>500) get full WDL. This teaches the net that large eval
/// differences should predict game outcomes.
...
fn main() {
    ...
    let mut trainer = ValueTrainerBuilder::default()
        ...
        // Dynamic WDL: scale with position decisiveness
        .wdl_adjust_function(|entry, wdl| {
            let abs_score = entry.score.unsigned_abs() as f32;
            // Scale: 0 at |score|=0, full WDL at |score|>=500
            let scale = (abs_score / 500.0).min(1.0);
            wdl * scale
        })
        .save_format(&[
            SavedFormat::id("l0w")
    ...
    let schedule = TrainingSchedule {
        net_id: "exp-f-wdl-dynamic".to_string(),
        eval_scale: 400.0,
        steps: TrainingSteps {
            batch_size: 16_384,
    ...
    println!("=== Experiment F: Dynamic WDL ===");
    println!("WDL scales with |score|: 0 at quiet, full at |score|>=500");
    println!("Base WDL=0.5, effective range 0.0-0.5");
