/// Experiment G: 1024 SCReLU v7 (known working arch) + Velvet training tricks
/// Architecture: (768×16 → 1024)×2 → SCReLU → concat 2048 → L1(16) → L2(32) → 1×8
/// Changes from previous v7_1024h16x32s: power-2.6 loss, weight decay 0.01, all 12 T80 files
///
/// Run: cargo run --release --example exp_g_v7_1024_tricks -- --superbatches 100
...
fn main() {
    ...
    let mut trainer = ValueTrainerBuilder::default()
        ...
        .build(|builder, stm_inputs, ntm_inputs, output_buckets| {
            ...
            // SCReLU on FT and both hidden layers (known working config)
    ...
    // Weight decay + stricter clipping for FT/factoriser
    ...
    let schedule = TrainingSchedule {
        net_id: "exp-g-v7-1024-tricks".to_string(),
        eval_scale: 400.0,
        steps: TrainingSteps {
            batch_size: 16_384,
    ...
    println!("=== Experiment G: v7 1024 SCReLU + Velvet Tricks ===");
    println!("1024 → SCReLU, L1=16, L2=32, 8 output buckets");
    println!("Power-2.6 loss, 12 T80 files, warmup {} SBs", warmup_sbs);
