/// Coda v5 1024 CReLU training config
/// Architecture: (768×16 → 1024)×2 → CReLU → concat 2048 → 1×8
/// Original baseline architecture.
///
/// Usage on GPU host:
///   cd ~/code/bullet
///   cargo run --release --example coda_v5_1024 -- \
///     --dataset /workspace/data/test80-2024-01-jan-2tb7p.min-v2.v6.binpack \
///     --superbatches 800 --wdl 0.07 --save-rate 100
///
/// Convert:
///   coda convert-bullet -v5 -input quantised.bin -output net.nnue
/// Name: net-v5-1024-w7-e800sNNN.nnue
...
fn main() {
    ...
    println!("=== Coda v5 1024 CReLU ===");
    println!("FT: {} → CReLU → concat {} → 1×{}", ft_size, 2 * ft_size, NUM_OUTPUT_BUCKETS);
